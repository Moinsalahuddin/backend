...
exports.getRequests = async (req, res) => {
  try {
    ...
    // Housekeeping staff can see requests assigned to them
...
exports.createRequest = async (req, res) => {
  try {
    ...
    // Notify admin/manager about new maintenance request
    try {
      await notificationController.notifyAllAdmins(
        'maintenance',
        'New Maintenance Request',
        `${populatedRequest.reportedBy.firstName} ${populatedRequest.reportedBy.lastName} reported ${populatedRequest.issueType} issue in Room ${populatedRequest.roomId.roomNumber} (Priority: ${populatedRequest.priority})`,
...
exports.updateRequest = async (req, res) => {
  try {
    ...
    // Admin/manager can update any status, housekeeping can update if assigned to them
    if (['assigned', 'in-progress', 'resolved', 'cancelled'].includes(req.body.status)) {
      if (req.user.role === 'housekeeping') {
        // Housekeeping staff can only update if assigned to them
